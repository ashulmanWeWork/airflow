...
class TaskInstance(Base, LoggingMixin):
    ...
    def error(self, session=None):
        ...
        if Marquez.is_enabled():
            Marquez.get_instance().update_job_run_state(self.run_id, State.FAILED)
        session.merge(self)
        session.commit()
...
class DAG(BaseDag, LoggingMixin):
    ...
    def create_dagrun(self,
        ...
        if Marquez.is_enabled():
            run_id = Marquez.get_instance("http://localhost", 8080).register_marquez_job_run(self)
            self.log.info("new run id is: %s", run_id)
        run = DagRun(
            dag_id=self.dag_id,
            run_id=run_id,
            execution_date=execution_date,
    ...
    def update_state(self, session=None):
        ...
        if len(tis) == len(dag.active_tasks):
            ...
            # if all roots succeeded and no unfinished tasks, the run succeeded
                ...
                self.state = State.SUCCESS
                if Marquez.is_enabled():
                    Marquez.get_instance().update_job_run_state(self.run_id, State.SUCCESS)
